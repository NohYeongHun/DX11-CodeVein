...
HRESULT CLoad_Mesh::Initialize_Vertex_For_Anim(std::ifstream& ifs)
{
	...
	// === 바운딩 박스 계산 (정점 데이터 로드 후) ===
	Calculate_MeshBoundingBox(info.vertices);
...
void CLoad_Mesh::Calculate_MeshBoundingBox(const vector<VTXANIMMESH>& vertices)
{
	// 바운딩 박스 초기화
	m_MeshBounding.vMin = { FLT_MAX, FLT_MAX, FLT_MAX };
	m_MeshBounding.vMax = { -FLT_MAX, -FLT_MAX, -FLT_MAX };

	// 모든 정점을 순회하며 바운딩 박스 계산
	for (const auto& vertex : vertices)
	{
		// 최소값 업데이트
		if (vertex.vPosition.x < m_MeshBounding.vMin.x) m_MeshBounding.vMin.x = vertex.vPosition.x;
		if (vertex.vPosition.y < m_MeshBounding.vMin.y) m_MeshBounding.vMin.y = vertex.vPosition.y;
		if (vertex.vPosition.z < m_MeshBounding.vMin.z) m_MeshBounding.vMin.z = vertex.vPosition.z;

		// 최대값 업데이트
		if (vertex.vPosition.x > m_MeshBounding.vMax.x) m_MeshBounding.vMax.x = vertex.vPosition.x;
		if (vertex.vPosition.y > m_MeshBounding.vMax.y) m_MeshBounding.vMax.y = vertex.vPosition.y;
		if (vertex.vPosition.z > m_MeshBounding.vMax.z) m_MeshBounding.vMax.z = vertex.vPosition.z;
	}
}
