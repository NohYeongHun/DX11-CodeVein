...
_bool CTool_Model::Picking(_float3* PickingPoint)
{
	...
	_bool bAnyHit = false;  // 전체 충돌 여부를 추적하는 변수

	// 가장 가까운 충돌 결과를 저장할 변수들
	_float3 vClosestHitPoint = {};
	_float3 vClosestNormal = {};

	// 모든 메시를 순회하면서 가장 가까운 충돌점 찾기
	for (_uint i = 0; i < m_iNumMeshes; i++)
	{
		_float3 vTempHitPoint = {};
		_float3 vTempNormal = {};
		_float fTempDist = 0.f;

		_bool bCurrentHit = m_Meshes[i]->Picking(vLocalRayOrigin, vLocalRayDir,
			&vTempHitPoint, &vTempNormal, &fTempDist);
		if (bCurrentHit)
		{
			bAnyHit = true;
			...
			{
				fMinDist = fTempDist;
				vClosestHitPoint = vTempHitPoint;  // 가장 가까운 충돌점 저장
				vClosestNormal = vTempNormal;      // 가장 가까운 법선 저장
			}
		}
	}

	// 충돌이 있었다면 가장 가까운 결과를 반환
	if (bAnyHit && PickingPoint)
	{
		*PickingPoint = vClosestHitPoint;
	}
