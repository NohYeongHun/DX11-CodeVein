...
NS_BEGIN(Client)
class CSwordWindCircle final : public CPartObject
{
public:
...
public:
    /* 클론시 전달할 정보들 */
    typedef struct tagSwordWindCircleDesc : public CPartObject::GAMEOBJECT_DESC
    {
    ...
    }SWORDWIND_CIRCLE_ACTIVATE_DESC;

private:
    explicit CSwordWindCircle(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
    explicit CSwordWindCircle(const CSwordWindCircle& Prototype);
    virtual ~CSwordWindCircle() = default;
...
private:
    SWORDWIND_CIRCLE_ACTIVATE_DESC m_ActivateDesc = {};
#pragma endregion
...
private:
    ...
    HRESULT Ready_Components(SWORDWIND_CIRCLE_DESC* pDesc);
#pragma endregion
...
public:
    static CSwordWindCircle* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
