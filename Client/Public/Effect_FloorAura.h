...
/* 실제 동작하는 Aura */
class CEffect_FloorAura final : public CPartObject
{
public:
...
public:
    /* 클론시 전달할 정보들 */
    typedef struct tagEffectFloorAuraDesc : public CPartObject::GAMEOBJECT_DESC
    {
        class CGameObject* pOwner = { nullptr };
        MESH_SHADERPATH eShaderPath = { MESH_SHADERPATH::NONE };
    }EFFECTFLOORAURA_DESC;

    /* POLL에서 Actviate 상태로 변할때 지정될 값들? */
    typedef struct tagFloorAuraEnterDesc
    {
    ...
    }EFFECTFLOORAURA_ACTIVATE_DESC;

private:
    explicit CEffect_FloorAura(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
    explicit CEffect_FloorAura(const CEffect_FloorAura& Prototype);
    virtual ~CEffect_FloorAura() = default;
...
private:
    EFFECTFLOORAURA_ACTIVATE_DESC m_ActivateDesc = {};
#pragma endregion
...
private:
    ...
    HRESULT Ready_Components(EFFECTFLOORAURA_DESC* pDesc);
#pragma endregion
...
public:
    static CEffect_FloorAura* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
