...
HRESULT CLoader_GamePlay::Loading_Resource(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, CGameInstance* pGameInstance)
{
	...
	{
		MSG_BOX(TEXT("Create Failed Loading : GamePlay GameObject "));
		return E_FAIL;
	}
	
	if (FAILED(Add_Prototype_Block(pDevice, pContext, pGameInstance)))
	{
		MSG_BOX(TEXT("Create Failed Loading : GamePlay Block "));
		return E_FAIL;
	}
...
HRESULT CLoader_GamePlay::Add_Prototype_Block(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, CGameInstance* pGameInstance)
{
	return S_OK;
}

HRESULT CLoader_GamePlay::Add_Prototype_Platform(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, CGameInstance* pGameInstance)
{
	return S_OK;
}
