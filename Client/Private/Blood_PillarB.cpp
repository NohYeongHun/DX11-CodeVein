...
CBlood_PillarB::CBlood_PillarB(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject{ pDevice, pContext }
{
}

CBlood_PillarB::CBlood_PillarB(const CBlood_PillarB& Prototype)
	: CPartObject(Prototype)
{
}

HRESULT CBlood_PillarB::Initialize_Prototype()
{
...
HRESULT CBlood_PillarB::Initialize_Clone(void* pArg)
{
...
#pragma endregion
	...
	if (FAILED(CGameObject::Initialize_Clone(pDesc)))
		CRASH("Failed Clone Blood PillarB");
...
void CBlood_PillarB::Priority_Update(_float fTimeDelta)
{
	CPartObject::Priority_Update(fTimeDelta);
}

void CBlood_PillarB::Update(_float fTimeDelta)
{
...
void CBlood_PillarB::Late_Update(_float fTimeDelta)
{
...
void CBlood_PillarB::OnActivate(void* pArg)
{
	PILLARB_ACTIVATE_DESC* pDesc = static_cast<PILLARB_ACTIVATE_DESC*>(pArg);
...
void CBlood_PillarB::OnDeActivate()
{
...
void CBlood_PillarB::Update_Grow(_float fTimeDelta)
{
	// 성장 완료 시, 다음 상태로 전환
...
void CBlood_PillarB::Update_Stay(_float fTimeDelta)
{
...
void CBlood_PillarB::Update_Decrease(_float fTimeDelta)
{
	// 감소 완료 시, 종료 상태로 전환
...
void CBlood_PillarB::RotateTurn_ToYaw(_float fTimeDelta)
{
	m_pTransformCom->Add_Rotation(0.f, fTimeDelta * XMConvertToRadians(90.f), 0.f);
}

HRESULT CBlood_PillarB::Bind_ShaderResources()
{
...
HRESULT CBlood_PillarB::Ready_Components(BLOODPILLAR_DESC* pDesc)
{
	/* 사용할 쉐이더.*/
	...
	if (FAILED(CGameObject::Add_Component(ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_Component_Model_Effect_BloodPillarB"),
...
CBlood_PillarB* CBlood_PillarB::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBlood_PillarB* pInstance = new CBlood_PillarB(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CBlood_PillarB"));
...
CGameObject* CBlood_PillarB::Clone(void* pArg)
{
	CBlood_PillarB* pInstance = new CBlood_PillarB(*this);
...
#ifdef _DEBUG
void CBlood_PillarB::ImGui_Render()
{
