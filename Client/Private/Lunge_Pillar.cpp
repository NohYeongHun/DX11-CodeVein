...
CLunge_Pillar::CLunge_Pillar(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject{ pDevice, pContext }
{
}

CLunge_Pillar::CLunge_Pillar(const CLunge_Pillar& Prototype)
	: CPartObject(Prototype)
{
}

HRESULT CLunge_Pillar::Initialize_Prototype()
{
...
HRESULT CLunge_Pillar::Initialize_Clone(void* pArg)
{
	LUNGEPILLAR_DESC* pDesc = static_cast<LUNGEPILLAR_DESC*>(pArg);
...
void CLunge_Pillar::Priority_Update(_float fTimeDelta)
{
...
void CLunge_Pillar::Update(_float fTimeDelta)
{
...
void CLunge_Pillar::Late_Update(_float fTimeDelta)
{
...
void CLunge_Pillar::OnActivate(void* pArg)
{
	...
	_float3 vScale = { m_fTargetRadius, m_fTargetRadius, m_fTargetHeight }; // Z가 높이임.

	m_eState = STATE_GROW;
	m_pTransformCom->Set_Scale(vScale);
}

void CLunge_Pillar::OnDeActivate()
{
...
void CLunge_Pillar::Calc_Timer(_float fTimeDelta)
{
	
}

void CLunge_Pillar::Shape_Control(_float fTimeDelta)
{
...
// z가 높이임.
void CLunge_Pillar::Update_Grow(_float fTimeDelta)
{
	// 성장 완료 시, 다음 상태로 전환
...
void CLunge_Pillar::Update_Stay(_float fTimeDelta)
{
...
void CLunge_Pillar::Update_Decrease(_float fTimeDelta)
{
	...
	OutputDebugWstring(TEXT("Lunge Pillar Size"));
...
void CLunge_Pillar::RotateTurn_ToYaw(_float fTimeDelta)
{
	m_pTransformCom->Add_Rotation( 0.f, fTimeDelta * XMConvertToRadians(90.f), 0.f );
}


HRESULT CLunge_Pillar::Bind_ShaderResources()
{
...
HRESULT CLunge_Pillar::Ready_Components(LUNGEPILLAR_DESC* pDesc)
{
	/* 사용할 쉐이더.*/
	...
	if (FAILED(CGameObject::Add_Component(ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_Component_Model_Effect_LungePillar"),
		TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom), &Desc)))
		CRASH("Failed LungePillar Components");

	/* 사용할 텍스쳐들 ?*/

	// 1. Diffuse 바인딩 => 늘수도있음. 아마 이건 0번 쓰겠지? 인덱스로
	if (FAILED(CGameObject::Add_Component(ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_Component_Texture_LungePillarDiffuse")
	...
	// 2. Other 바인딩
	if (FAILED(CGameObject::Add_Component(ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_Component_Texture_LungePillarOther")
	...
	// 3. Noise 바인딩
	if (FAILED(CGameObject::Add_Component(ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_Component_Texture_LungePillarNoise")
...
CLunge_Pillar* CLunge_Pillar::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLunge_Pillar* pInstance = new CLunge_Pillar(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CLunge_Pillar"));
...
CGameObject* CLunge_Pillar::Clone(void* pArg)
{
	CLunge_Pillar* pInstance = new CLunge_Pillar(*this);
...
#ifdef _DEBUG
void CLunge_Pillar::ImGui_Render()
{
